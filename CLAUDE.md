...
# Build the Word CV (same SSOT) → CV/Curriculum Vitae Chihun Lee (YYYY.MM).docx
/opt/homebrew/Caskroom/miniforge/base/envs/3dp/bin/python CV/build_cv_docx.py
...
- The user's existing `CV/Curriculum Vitae Chihun Lee (2025.11).docx` — pre-2026 baseline of contact info, projects, patents; kept for reference, do not modify.
  The current Word CV is `CV/Curriculum Vitae Chihun Lee (2026.09).docx`, generated by `CV/build_cv_docx.py` from the same YAML — never hand-edit it, edit `data/*.yaml` and rebuild.
